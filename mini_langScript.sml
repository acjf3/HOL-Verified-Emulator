...
val _ = Hol_datatype `
  MINI_INSTRUCTION =
    ...
    (* execute NOP and skip to next MINI_MARKER, in case condition not met *)
      MINI_CONDITIONAL of word4 => word32
    ...
    (* arithmetic/logic *)
    | MINI_ADD_IMM of word2 => word32 (* simple addition, e.g. for pc update *)
...
val num2binop_thm = save_thm("num2binop_thm",
  find_terms (numSyntax.is_numeral) (concl num2binop_def) |> rev
  |> map (fn tm => EVAL (``num2binop ^tm``)) |> LIST_CONJ);
...
val num2assert_thm = save_thm("num2assert_thm",
  find_terms (numSyntax.is_numeral) (concl num2assert_def) |> rev
  |> map (fn tm => EVAL (``num2assert ^tm``)) |> LIST_CONJ);
...
val eval_mini_def = Define `
  ...
  (eval_mini (MINI_CONDITIONAL cond imm) (regs,ok,s) =
     if arm_condition_passed cond (ARM_READ_NZCV s) then
       (F,regs,ok,s)
     else
       (T,regs,ok,ARM_WRITE_REG 15w (ARM_READ_REG 15w s + imm) s)) /\
  ...
  (eval_mini (MINI_ADD_IMM a imm32) (regs,ok,s) =
     (F,(a =+ regs a + imm32) regs,ok,s)) /\
